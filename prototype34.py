...
# spare API Key - AIzaSyAyR0scmCah89IF6SdLPXt61WGCMfC4mfI
...
def update_traffic_lights(root, road_names, snapped_points, traffic_lights, api_key, life_cycle_seconds):
    ...
    for index in range(len(sorted_indices)):
        ...
        if index == len(sorted_indices) - 1:
            ...
            traffic_thread = threading.Thread(target=fetch_new_traffic_data, args=(snapped_points, api_key, new_data_holder))
    ...
    root.after(1000, update_traffic_lights, root, road_names, snapped_points, traffic_lights, api_key, life_cycle_seconds)
